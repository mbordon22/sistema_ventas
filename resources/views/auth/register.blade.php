<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <!-- Numero DNI -->
        <div class="mt-4">
            <x-input-label for="numero_dni" :value="__('DNI')" />
            <x-text-input id="numero_dni" class="block mt-1 w-full" type="text" name="numero_dni" :value="old('numero_dni')" required autofocus autocomplete="numero_dni" />
            <x-input-error :messages="$errors->get('numero_dni')" class="mt-2" />
        </div>

        <!-- Numero Telefono -->
        <div class="mt-4">
            <x-input-label for="telefono" :value="__('N° Telefono')" />
