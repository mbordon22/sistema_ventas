    ...
    <div class="w-3/4 max-w-screen-xl mx-auto py-6 sm:px-6 lg:px-8">    
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mt-4">
            ...
            @if ($producto->subcategoria->talle == 1 && $producto->subcategoria->color == 0)
                @livewire('editar-variante-talle', ['producto' => $producto], key($producto->id))
            @endif

            {{-- Solo Color --}}
            @if ($producto->subcategoria->color == 1 && $producto->subcategoria->talle == 0)
                @livewire('editar-variante-color', ['producto' => $producto], key($producto->id))
            @endif

            {{-- Color y Talle --}}
            @if ($producto->subcategoria->color == 1 && $producto->subcategoria->talle == 1)
                @livewire('editar-variante-color-talle', ['producto' => $producto], key($producto->id))
            @endif
        </div>
    </div>    
</x-app-layout>
